...
class DBN(object):
	def __init__(self, layers, n_labels):
		...
		self.mlp = MLP(act_type='Sigmoid', opt_type='Adam', layers=layers+[n_labels], epochs=20, learning_rate=0.01, lmbda=1e-2)
...
def main():
	...
	print('dbn initialization train acc', sum(np.argmax(dbn.predict(train_x), axis=1) == train_y)/train_y.shape[0])
	print('dbn initialization test acc', sum(np.argmax(dbn.predict(test_x), axis=1) == test_y)/test_y.shape[0])

	print('mlp training')
	mlp = MLP(act_type='Sigmoid', opt_type='Adam', layers=[train_x.shape[1], 100, 100, 10], epochs=20, learning_rate=0.01, lmbda=1e-2)
	mlp.fit(train_x, train_y)
	print('mpl train acc', sum(np.argmax(mlp.predict(train_x), axis=1) == train_y)/train_y.shape[0])
	print('mpl test acc', sum(np.argmax(mlp.predict(test_x), axis=1) == test_y)/test_y.shape[0])
